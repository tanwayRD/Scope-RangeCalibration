        ...
        while True:
            if flag == 1:
                if distanceTrue > slow_limit:
                    ...
                    for n in range(iteration):
                        sock = data[n]  #1个UDP包1440B
                        for i in range(20): #20个角度，每个角度72个B
                            sub_sock = sock[72 * i:72 * i + 72]
                            hor_angle_tem = sub_sock[64:68]
                            reserve = sub_sock[68:]
                            hor_angle = int.from_bytes(hor_angle_tem, byteorder='big', signed=False) / 100000
                            if 91 >= hor_angle >= 89:
                                for j in range(16):
                                    width_tem = sub_sock[2 + j * 4:4 + j * 4]
                                    distance_tem = sub_sock[0 + j * 4:2 + j * 4]
                                    ...
                                    file.writelines('Channel: ' + str(j+1) + ' Angle: ' + str(hor_angle) + ' Distance: ' \
