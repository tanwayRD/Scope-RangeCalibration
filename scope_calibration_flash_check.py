...
class Qmycali(QWidget):
    ...
    @pyqtSlot()
    def on_pushButton_clicked(self):  #获取原始数据路径
      #print("test")
    ...
    @pyqtSlot()
    def on_pushButton_3_clicked(self):  #导入配置文件
      #print("test_hahah")
      self.ui.config_path = QFileDialog.getOpenFileName(self, "getOpenFileName","./","All Files (*);;Text Files (*.txt)")
    ...
    def on_pushButton_2_clicked(self):  #
        ...
        for line in file:
            count += 1
            line_mod = line.replace(' ', '')    #取消空格
            if count < 67:
                oneline = re.split('=|\n',line_mod)
                if count == 1:
                    function = int(oneline[1])  # 1: 引入自动范围功能 0：不引入自动范围功能
                elif count == 2:
                    blind = float(oneline[1])
                else:
                    leftbound[count-3] = float(oneline[1])
